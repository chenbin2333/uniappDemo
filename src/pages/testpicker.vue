  ...
  <view>
    <view v-for='(item, index) in arr' :key='index'>
      <view class="uni-list-cell">
        ...
        <view class="uni-list-cell-db">
          <picker v-if='item.componentType === "normal"' @change="(e) => bindPickerChange(e, index)" :value="item.index" :range="item.componentValue">
            <view class="uni-input">{{item.index === -1 ? "请选择" : item.componentValue[item.index]}} <text class='text'> > </text> </view>
          </picker>
          <picker v-if='item.componentType === "date"' mode="date"
                  :value="item.date" :start="item.startDate" :end="item.endDate" @change="(e) => bindDateChange1(e, index)">
          ...
          <view v-if='item.componentType === "dialog"' class="dialog-box">
            <view class='item' @click='popUp(index)'>{{item.index === -1 ? "请选择" : item.componentValue[item.index]}} <text class='text'> > </text> </view>
          </view>
        </view>
      </view>
    </view>
    ...
    <uni-popup ref="popup" background-color="#fff">
      <view class="popup-content" style="padding: 10px">
        定时重复上报
        <view class='dscfsb'>
          <view class='dscfsb-item' @click='clickItem(index)' :class='{active: index === currentIndex}' v-for='(item, index) in dscfsb' :key='index'>
            {{item}}
          </view>
        </view>
       <view>
         <view v-if='currentIndex === 2'>
           <view>日期</view>
           <view  class='week-arr'>
             <view @click='clickWeek(index)' v-for='(item, index) in dateArr' class='week-item' :key='index'>
               {{item.name}}
             </view>
           </view>
         </view>
